<?php
namespace Laralite\Form;

use Closure;
use Laralite\Form\Exceptions\InvalidFrameworkException;
...
/**
 * Helps the user interact with the various Form components
 */
class Form
{
	// Instances
	 ...
	 */
	const FORMSPACE = 'Form\Form\\';

	/**
	 * The namespace of fields
	 */
	const FIELDSPACE = 'Form\Form\Fields\\';
	...
	{
		$this->app['form.populator']->replace($values);
	}
	...
	{
		$this->app['form.populator']->put($field, $value);
	}
	...
	{
		return $this->app['form.populator']->get($field, $fallback);
	}
	...
	public function withRules()
	{
		...
		foreach ($rules as $name => $fieldRules) {
			...
			foreach ($expFieldRules as $rule) {
				...
				// Store processed rule in Form's array
				if (!isset($parameters)) {
					$parameters = array();
				}
	 ...
	 * @return \Form\Interfaces\FrameworkInterface
	 */
	public function getFrameworkInstance($framework)
	{
		$formClass = __NAMESPACE__.'\Framework\\'.$framework;

		//get interfaces of the given framework
		$interfaces = class_exists($framework) ? class_implements($framework) : array();

		if(class_exists($formClass)) {
			$returnClass = $formClass;
		} elseif(class_exists($framework) && isset($interfaces['Form\Interfaces\FrameworkInterface'])) {
	...
	{
		return $this->app['config']->get('form.'.$option, $default);
	}
	...
	{
		return $this->app['config']->set('form.'.$option, $value);
	}
	...
	public function getErrors($name = null)
	{
		...
		if (!$name and $this->app['form.field']) {
			$name = $this->app['form.field']->getName();
