<?php
namespace Laralite\Form\Traits;
...
abstract class Checkable extends Field
{
	...
	public function render()
	{
		...
		// Multiple items
		if ($this->items) {
			unset($this->app['form']->labels[array_search($this->name, $this->app['form']->labels)]);
	...
	protected function createCheckable($item, $fallbackValue = 1)
	{
		...
		if (!isset($value) or $value === $this->app['form']->getOption('unchecked_value')) {
			$value = $fallbackValue;
		}

		// If inline items, add class
		$isInline = $this->inline ? ' '.$this->app['form.framework']->getInlineLabelClass($this) : null;

		// In Bootsrap 3, don't append the the checkable type (radio/checkbox) as a class if
		// rendering inline.
		$class = $this->app['form']->framework() == 'TwitterBootstrap3' ? trim($isInline) : $this->checkable.$isInline;
		...
		if ($this->isOfType('checkbox', 'checkboxes')) {
			if ($this->isPushed or ($this->app['form']->getOption('push_checkboxes') and $this->isPushed !== false)) {
				$field = $this->app['form']->hidden($name)->forceValue($this->app['form']->getOption('unchecked_value')).$field->render();

				// app['form.field'] was overwritten by Form::hidden() call in the line above, so here
				// we reset it to $this to enable $this->app['form']->getErrors() to retrieve the correct object
				$this->app->instance('form.field', $this);
			}
		}
		...
		if (!$isInline && $this->app['form']->framework() == 'TwitterBootstrap3') {
	...
	protected function isChecked($item = null, $value = null)
	{
		...
		if ($this->isGrouped()) {
			...
			$static = $this->app['form']->getValue($this->bind ?: $this->name);
			...
			$post   = $this->app['form']->getPost($name);
			$static = $this->app['form']->getValue($this->bind ?: $name);
		}

		if (!is_null($post) and $post !== $this->app['form']->getOption('unchecked_value')) {
