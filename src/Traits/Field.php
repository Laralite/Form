...
use Laralite\Form\Form\Form;
use Laralite\Form\Form\Group;
use Laralite\Form\Form;
use Laralite\Form\Helpers;
use Laralite\Form\Interfaces\FieldInterface;
use Laralite\Form\LiveValidation;
 ...
 */
abstract class Field extends FormObject implements FieldInterface
{
	/**
	 * The IoC Container
	 *
	...
	{
		if ($this->app->bound('form.form.framework')) {
			return $this->app['form.form.framework'];
		}

		return $this->app['form.framework'];
	}
	...
	public function __construct(Container $app, $type, $name, $label, $value, $attributes)
	{
		...
		$this->form = $this->app->bound('form.form') ? $this->app['form.form'] : null;
		...
		if ($this->app['form']->getOption('live_validation')) {
		...
		$groupClass = Form::FORMSPACE.$groupClass;
	...
	{
		return $this->app['form']->getRules($this->name);
	}
	...
	public function label($text, $attributes = array())
	{
		...
		$label = $this->app['form']->label($text, $for, $attributes);
	...
	private function automaticLabels($name, $label)
	{
		...
		if (!$this->app['form']->getOption('automatic_label')) {
			$this->name = $name;
			$this->label($label);
