...
/**
 * Dispatch calls from Form to the different
...
class MethodDispatcher
{
	...
	{
		if (!$this->app['form']->hasMacro($method)) {
			return false;
		}

		// Get and format macro
		$callback = $this->app['form']->getMacro($method);
	...
	protected function getClassFromMethod($method)
	{
		...
		switch ($method) {
			...
			case 'link':
			case 'reset':
				$class = Form::FIELDSPACE.'Button';
				break;

			case 'multiselect':
				$class = Form::FIELDSPACE.'Select';
				break;

			default:
				$class = Form::FIELDSPACE.'Input';
				break;
		}
