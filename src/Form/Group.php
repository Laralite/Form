<?php
namespace Laralite\Form\Form;

use BadMethodCallException;
use Laralite\Form\Helpers;
...
class Group extends Tag
{
	...
	public function __construct(Container $app, $label, $validations = null)
	{
		// Get special classes
		$this->app = $app;
		$this->addClass($this->app['form.framework']->getGroupClasses());

		// Invisible if Nude
		if ($this->app['form.framework']->is('Nude')) {
			$this->element = '';
		}
	...
	{
		if ($this->getErrors()) {
			$this->state($this->app['form.framework']->errorState());
		}
		...
		// Required state
		if ($this->app->bound('form.field') and $this->app['form.field']->isRequired()) {
			$this->addClass($this->app['form']->getOption('required_class'));
		}
	 ...
	 *
	 * @param  \Form\Traits\Field $field A Field instance
	 *
	 * @return string        A group
	 */
	...
	{
		// Filter state
		$state = $this->app['form.framework']->filterState($state);
	...
	public function getFormattedLabel()
	{
		...
		return $this->label->addClass($this->app['form.framework']->getLabelClasses());
	}
	...
	public function inlineHelp($help, $attributes = array())
	{
		...
		$this->help['inline'] = $this->app['form.framework']->createHelp($help, $attributes);
	}
	...
	{
		// Reserved method
		if ($this->app['form.framework']->isnt('TwitterBootstrap') && $this->app['form.framework']->isnt('TwitterBootstrap3')) {
		...
		$this->help['block'] = $this->app['form.framework']->createBlockHelp($help, $attributes);
	}
	...
	{
		$icon = $this->app['form.framework']->createIcon($icon, $attributes, $iconSettings);
	...
	public function wrap($contents, $label = null)
	{
		$group = $this->open();
		$group .= $label;
		$group .= $this->app['form.framework']->wrapField($contents);
		$group .= $this->close();
	...
	protected function getLabel($field = null)
	{
		...
		$this->label->addClass($this->app['form.framework']->getLabelClasses());
		$this->label = $this->app['form.framework']->createLabelOf($field, $this->label);
		$this->label = $this->app['form.framework']->wrapLabel($this->label);
	...
	protected function getHelp()
	{
		...
		$errors = $this->app['form']->getErrors();
		if ($errors and $this->app['form']->getOption('error_messages')) {
			$inline = $this->app['form.framework']->createHelp($errors);
		}
	...
	protected function prependAppend($field)
	{
		...
		return $this->app['form.framework']->prependAppend($field, $this->prepend, $this->append);
	}
	...
	protected function placeAround($items, $place)
	{
		...
		foreach ((array) $items as $item) {
			$item             = $this->app['form.framework']->placeAround($item);
			$this->{$place}[] = $item;
		}
	}
}
