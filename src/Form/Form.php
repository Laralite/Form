...
use Laralite\Form\Populator;
use Laralite\Form\Traits\FormObject;
...
class Form extends FormObject
{
	...
	public function __construct(Container $app, $url, Populator $populator)
	{
		...
		$this->app->singleton('form.form.framework', function ($app) {
			return clone $app['form.framework'];
		});
	}
	...
	public function openForm($type, $parameters)
	{
		...
		if ($this->app['form']->getOption('fetch_errors')) {
			$this->app['form']->withErrors();
		}
		...
		if ($this->type !== 'raw') {
			$this->addClass($this->app['form.form.framework']->getFormClasses($this->type));
		}
	...
	public function close()
	{
		...
		$closing = '</form>';
		if ($this->method != 'GET') {
			$closing = $this->app['form']->token().$closing;
		}
	...
	public function __toString()
	{
		...
		if (in_array($this->method, array('PUT', 'PATCH', 'DELETE'))) {
			$spoof        = $this->app['form']->hidden('_method', $this->method);
			$this->method = 'POST';
		} else {
			$spoof = null;
		}
	...
	/**
	 * Alias for $this->app['form']->withRules
	 */
	public function rules()
	{
		call_user_func_array(array($this->app['form'], 'withRules'), func_get_args());
	...
	private function applyType($type)
	{
		// If classic form
		if ($type == 'open') {
			return $this->app['form']->getOption('default_form_type');
		}
		...
		// If raw form
		if ($type == 'raw') {
			$this->app->bind('form.form.framework', function ($app) {
				return $app['form']->getFrameworkInstance('Nude');
			});
		}

		// Use default form type if the one provided is invalid
		if ($type !== 'raw' and !in_array($type, $this->app['form.form.framework']->availableTypes())) {
			$type = $this->app['form']->getOption('default_form_type');
		}
